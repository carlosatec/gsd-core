<!-- GSD-SESSION-CONTEXT -->
> **GSD Active State**: Phase: Phase 21 (Zero-Config Bootstrap, RRF Semantic RAG & Multi-Agent Concurrency Hardening) | Status: Complete (Fase 21 concluída com 100% dos testes e guardrails verificados)
> **Decisions**: **D-01 [Architecture]:** Analisador AST nativo em TypeScript sem dependência de Python ou compiladores externos.; **D-02 [Docs]:** Documentação viva incremental (`ARCHITECTURE.md`, `APIS.md`) sincronizada com o código.; **D-03 [Context]:** JIT context injection para montar prompts cirúrgicos baseados no grafo de dependências.; **D-04 [Guardrails]:** Pre-flight static verification + self-healing loop automático em caso de falhas de teste.; **D-104 [Zero-Config Bootstrap]:** Seam gerenciado de compilação nos hooks npm.; **D-105 [RRF RAG]:** Fusão invariante de escala BM25 + PageRank com salvaguarda de neutralidade.; **D-106 [Concurrency Lock]:** Trava atômica com evicção TTL (5s) e verificação de PID vivo.; **D-107 [Guardrail Normalization]:** Normalização rigorosa contra bypass por prefixo `./`.
