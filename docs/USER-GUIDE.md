...
A narrative companion guide to GSD Core Nexus 3.3 — orient yourself here, then follow the links into the dedicated docs.
...
Starting with GSD Core Nexus 3.3, the public command surface is streamlined into **10 Canonical Unified Commands**. All operational sub-skills and internal workflows are orchestrated seamlessly under these entrypoints:
...
| `/gsd-migrate` | Non-destructive upgrade for legacy and greenfield projects to GSD Core Nexus 3.3 | `--dry-run`, `--force` |
...
GSD Core Nexus 3.3 automatically records structured, append-only JSONL execution events under `.planning/intel/sessions/`. Every command execution captures tool invocations, AST pre-flight checks, stack traces, and real diffs:
