...
| `canvas-roadmap-generator.cjs` | Visual Canvas Roadmap Exporter (Obsidian Canvas `.canvas` JSON specification with phase status colors and dependency edges) |
...
| `obsidian-interop.cjs` | Obsidian Interoperability & Bidirectional Wikilinks Engine (`[[wikilinks]]` extraction, target resolution, and `.planning/intel/backlinks.json` index) |
...
| `visual-graph-exporter.cjs` | Standalone Visual Knowledge Graph Exporter (100% offline interactive HTML5/Canvas 2D graph viewer, PageRank sizing, real-time search, filters, and PNG snapshot export) |
