# 🚀 Practical Tutorial: Mastering GSD Core Nexus 2.8

> 🌐 **Language / Idioma:** **English** | [Versão em Português (Brasil)](../pt-BR/tutorials/tutorial-pratico.md)  
> **Git. Ship. Done.**  
> The definitive guide to autonomous software engineering, meta-prompting, surgical context injection, causal session intelligence, and AI governance with **GSD Core Nexus 2.8**.
...
/gsd:migrate
```
Upgrades directory layouts, manifests, and schema versions to GSD Core Nexus 2.8 non-destructively.

---

## 4. The 10 Canonical Unified Commands Surface

In GSD 2.7, the user-facing command surface is strictly consolidated into **10 canonical commands**, while all operational playbooks are dynamically loaded on-demand via execution context:

```text
┌─────────────────────────────────────────────────────────────┐
│                 GSD 2.8 CANONICAL INTERFACE                 │
├────────────┬────────────────────────────────────────────────┤
...
2. **`/gsd:plan [N]` — Atomic Planning with AST & Specs:**
   - **What it does:** Runs AST analysis on codebase, calculates PageRank and Okapi BM25 indices in `.planning/intel/`, aligns technical decisions in `SPEC.md`, and decomposes the phase into atomic task waves in `PLAN.md`.
   - **When to use:** Before starting any new phase or feature development.

3. **`/gsd:exec [N]` — Wave Execution with JIT Context:**
   - **What it does:** Executes tasks wave by wave. Runs pre-flight static guardrails, injects minimal type signatures and dependencies via JIT, and dispatches subagents with clean 200k context windows that commit atomically.
   - **When to use:** Immediately after reviewing and approving the plan generated by `/gsd:plan`.

4. **`/gsd:review [--fix]` — Static Analysis & Autonomous Repair:**
   - **What it does:** Audits all modified files in the phase for style defects, excessive cyclomatic complexity, and anti-patterns. With `--fix`, automatically applies autonomous repairs.
...
8. **`/gsd:tokens` — Token Telemetry & Observability Dashboard:**
   - **What it does:** Renders a 65-column responsive ASCII dashboard showing real-time metrics: total invocations, JIT savings rate (80-90% average), peak consumption bursts, and per-command breakdown.
...
9. **`/gsd:migrate` — Non-Destructive Project Modernization:**
   ...
   - **When to use:** When bringing an existing project from older GSD versions to GSD Nexus 2.7.
...
GSD Core Nexus 2.8 includes an automated, single-command release orchestrator (`scripts/bump-version.cjs`):
...
*GSD Core Nexus 2.8 — Develop with surgical precision, zero context rot, causal observability, and peak efficiency.*
