/**
 * Auto-Upgrade Engine — Seamless non-destructive upgrade from legacy projects to GSD Core Nexus 3.4.
 *
...
function runAutoUpgrade(planningDir: string, rootDir?: string, options?: AutoUpgradeOptions): AutoUpgradeReport {
  ...
  if (options?.dryRun) {
    return {
      ...
      docsUpdated: 0,
      generatedArtifacts: [],
      message: `[Dry-Run] Projected upgrade to GSD Core Nexus 3.4: would index ${graph.stats.totalFiles} files across [${detectedLanguages.join(', ')}]${fwText}. No files were written.`,
    };
  }
  ...
  return {
    ...
    message: isNewMigration
      ? `Successfully upgraded project to GSD Core Nexus 3.4. Indexed ${graph.stats.totalFiles} files across [${detectedLanguages.join(', ')}]${fwText}. Generated ${syncReport.generatedDocs.length} living doc(s).`
      : `Refreshed GSD Core Nexus 3.4 intelligence layer for ${graph.stats.totalFiles} files (${syncReport.generatedDocs.length} doc(s) updated).`,
  };
}
