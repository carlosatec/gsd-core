 ...
 */
function rankDecisions(decisions, targetFiles, query, neighborFiles = [], maxDecisions = 5) {
    ...
    for (const f of targetFiles) {
        const stem = node_path_1.default.basename(f, node_path_1.default.extname(f)).toLowerCase();
        if (stem.length > 2) {
            fileStems.push(stem);
        ...
        const dirParts = node_path_1.default.dirname(f).toLowerCase().split(/[/\\]+/);
...
function assembleJitContext(options) {
    ...
    if (stateContent) {
        ...
        applicableDecisions.push(...rankDecisions(rawCandidates, targetFiles, options.query, allNeighbors.map(n => n.file), maxDecisions));
    }
