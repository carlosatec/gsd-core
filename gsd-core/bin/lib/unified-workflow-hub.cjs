"use strict";
/**
 * Unified Workflow Hub — Streamlined 6+1 Command Surface & Reviewer for GSD Core Nexus 3.3.
 *
 ...
 */
function executeReview(planningDir, rootDir, autoFix = false, explicitFiles, fullRepoMode = false, phaseId) {
    const fixed = [];
    ...
    if (explicitFiles && explicitFiles.length > 0) {
        ...
        if (phaseFiles.length > 0) {
            ...
            filesToReview = topCentral.length > 0 ? topCentral.map((t) => t.file) : Object.keys(graph.files).slice(0, 5);
        }
    }
    ...
    try {
        ...
        if (fullRepoMode) {
            recordedTelemetry = recordJitInvocation(resolvedPlanningDir, filesToReview.slice(0, 10), repoTokenBaseline, repoTokenBaseline, 'review', activePhase, undefined, 'full-repo');
        }
        else {
            const baseline = Math.max(repoTokenBaseline, reviewedTokens * 5);
            recordedTelemetry = recordJitInvocation(resolvedPlanningDir, filesToReview, reviewedTokens, baseline, 'review', activePhase, undefined, 'targeted');
        }
    }
    catch {
 ...
 */
function resolveActivePhaseId(planningDir, explicitPhaseOrArgs) {
    if (typeof explicitPhaseOrArgs === 'string' && explicitPhaseOrArgs.trim()) {
        ...
        for (const arg of explicitPhaseOrArgs) {
            if (typeof arg === 'string' && !arg.startsWith('-')) {
                ...
                if (!CANONICAL_COMMAND_SET.has(lower) && lower !== 'review' && lower !== 'plan' && lower !== 'exec' && lower !== 'verify') {
    ...
    switch (canonicalName) {
        case 'auto':
            ...
            return {
                ...
                message: 'GSD Core Nexus 3.3 Autopilot active. Running phase loop with guardrails.',
            };
        case 'status': {
            ...
            return {
                ...
                data: { telemetry },
                message: `GSD Core Nexus 3.3 Status analyzed. Context and phase roadmap verified.${teleMsg}`,
            };
        }
        case 'plan': {
        ...
        case 'review': {
            const phaseId = resolveActivePhaseId(planningDir, options.args);
            const isFull = Boolean(options.flags?.['full'] ||
                options.flags?.['repo'] ||
                options.flags?.['fullRepo'] ||
                options.args.includes('--full') ||
                options.args.includes('--repo'));
            const explicitFiles = options.args.filter(arg => !arg.startsWith('-') &&
                arg !== 'review' &&
                arg !== phaseId &&
                (arg.includes('/') || arg.includes('\\') || arg.includes('.')));
            const reviewResult = executeReview(planningDir, cwd, hasFixFlag, explicitFiles.length > 0 ? explicitFiles : undefined, isFull, phaseId);
